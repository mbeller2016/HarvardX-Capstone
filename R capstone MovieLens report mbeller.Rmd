...
The model's effectiveness was judged by its ability to minimize root mean square error (RMSE).  A specific target of RMSE < 0.8649 was specified.  As discussed in [***Section 3***](#R3) (Results), RMSE was computed on the **edx_test** set for several iterative versions of the model.  The final version was then run against the **validation** set, and the target was achieved.  The final result from the validation run can be found in [***Section 3.5***](#R3.5).
...
Having quantified the main effects above, the first test of the model was performed on the test data set.  The specifics of the test are shown in [***Section 3.1***](#R3.1).  Overall, the model performed well, clearing the target RMSE threshold with a little room to spare.
...
A prediction run against the test set was done with this model, as discussed in [***Section 3.2***](#R3.2).  The results were a modest improvement from the unregularized version.
...
A prediction run against the test set was done with this model, as discussed in [***Section 3.3***](#R3.3).  The results were a further modest improvement from the previous version.
...
A prediction run against the test set was done with this model, as discussed in [***Section 3.4***](#R3.4).  The results were again a modest improvement from the previous version.

# 3. Results {#R3}

This section includes the code and results for the four versions of the model run on the test data set, as well as the final version's results against the validation set.  To see the specific results for the final validation set, skip to [***Section 3.5***](#R3.5).
...
## 3.1 Results - **edx_test**, Full Model, no regularization {#R3.1}
...
## 3.2 Results - **edx_test**, Full Model, movie effects regularized {#R3.2}
...
## 3.3 Results - **edx_test**, Full Model, movie effects and user/genre effects regularized {#R3.3}
...
## 3.4 Results - **edx_test**, Full Model, movie effects and user/genre effects regularized, user pickiness adjusted {#R3.4}
...
## 3.5 Final Result - Validation set, Full Model, movie effects and user/genre effects regularized, user pickiness adjusted {#R3.5}

The model from 3.4 was the best performer on the **edx_test** data set:

**Predicted Rating = OMR + MEr + UGEr + UPEa**

See the beginning of [***Section 3***](#R3) for explanations of these terms.
